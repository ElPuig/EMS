...
**Second, separate trigger for the same tool: send a proactive notification whenever you finish all the work requested for the current task, not only when a test run risks hanging.** The developer may have stepped away while you worked through a multi-step change; a chat message alone is silent until they come back and think to look. So once every part of the current request is done (code changed, tests run, docs/i18n updated — whatever the task actually required), send one short push/desktop notification summarizing what changed, so they know it's ready to review. Call the notification tool and let it make the judgment call on whether delivery is actually warranted right now (e.g. it may skip sending if it detects the developer is actively watching the session) — don't pre-judge that for it by skipping the call yourself. If the developer's session runs inside a container, this is the same delivery problem as the test-hang notification below, and the same fix applies — see the container-bridge section that follows, and `docs/en/developers/tooling/ai_agent_test_notifications.md`.

**The notification tool can also self-suppress ("terminal active, a separate notification would be redundant") — for the test-hang trigger specifically, that judgment is often wrong, so don't treat a skip as "handled."** The tool's heuristic assumes that if the developer can already see your text output, a desktop notification would just duplicate it. That holds for information ("here's what I found"), but the test-hang notification isn't information — it's a prompt for a physical action in a *different* window (refreshing an Odoo browser tab) that the developer has no reason to go do just because they can read the chat. Confirmed in practice: the run genuinely hung and only proceeded once the developer noticed on their own and refreshed — exactly the failure mode the notification exists to prevent. There is no override for this from the calling side; still call the tool every time (it may go through when it wouldn't for a pure-info message), but also always pair it with a clearly-worded in-chat instruction to refresh the tab, since the desktop notification is not guaranteed to fire even when it's genuinely needed.
...
4. **Container-side Claude Code hook** — in `~/.claude/settings.json` (user-level, **not** the project's checked-in `.claude/settings.json`, since the mount path/username here are specific to this one developer's machine and would break for everyone else cloning the repo):
   ...
   **This hook only covers the test-hang trigger.** There is no equivalent Bash command to match on for the task-completion trigger — "all requested work is done" isn't a tool call, it's a judgment the agent makes at the end of a turn. So for that trigger, once the bridge above is set up, the agent should write directly into the shared drop directory itself (e.g. `echo "$(date +%H:%M:%S) EMS: task done — <short summary>" > /mnt/claude-notify/done-$(date +%s%N).txt`) right after concluding a task, instead of (or in addition to) calling `PushNotification` — the same watcher picks it up either way.

Proactively offer this whole bridge — don't wait to be asked — whenever a developer mentions they're not noticing test-hang reminders or task-completion notifications, and their session is running inside a container rather than a bare-metal/VM terminal CLI install; that combination is the specific signature this fix addresses. It's real setup work (shared directory, permissions, a host script, a systemd unit, a hook) — walk through it step by step and verify each layer (write test, then a real hook-triggered test) before declaring it done, the same way it was built and verified here.
