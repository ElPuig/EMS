...
Nothing told the frozen values of the outcome grid apart from the grade the operator is meant to type: Odoo only marks a read-only cell once its row is in edit mode, so until the first click every column looked equally editable. The current grade is now muted and the grade being resolved is emphasised, and the column that was purely informative is hidden by default. The column headers were also losing their last characters to an ellipsis - Odoo gives numeric columns a fixed narrow width and truncates rather than wrapping a header that does not fit, and no width relaxes it - so they now use a second line, the same way the work placement matrix already does. The same treatment fixes the subjects list of the course record itself, where several headers were equally unreadable, and the work placement column there is now simply labelled "EM". The dialog also opens wider, which gives the learning outcomes' own names room to be read.
