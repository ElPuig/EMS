...
6. Fes clic a **Continua** a les pantalles restants (encara no construïdes com a pantalles pròpies — una versió futura permetrà resoldre aquí mateix els conflictes d'aula, en lloc de només al pas final).
7. Al pas final, fes clic a **Importa**. Aquest és el moment en què tot s'escriu de debò, i on qualsevol problema pendent (una aula que falta, un conflicte d'horari) es reporta indicant exactament què cal corregir.
