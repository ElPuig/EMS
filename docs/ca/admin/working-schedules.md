...
5. Si els fitxers esmenten un correu de docent que EMS no ha trobat, una pantalla de **Resoldre docents** en llista cadascun, amb **Nou** marcat per defecte (assumint un docent genuïnament mai contractat) - deixa-ho marcat per crear un nou docent pendent d'identificació per a aquest correu al pas final d'Importar, de la mateixa manera que un codi de lloc encara no cobert (vegeu "Docents encara no contractats" més avall), amb la diferència que es conserva el correu del fitxer, precarregat com a **Correu de treball** editable a mà (**Assignar correu corporatiu manualment** marcat) en lloc de generar-se automàticament, ja que encara no s'ha confirmat; o bé, si en realitat és un error/desajust d'un docent ja existent, desmarca **Nou** i tria el docent real al desplegable (desmarcar-lo és el que el desbloqueja). Si tots els correus s'han reconegut, veuràs un missatge de confirmació. El botó **Continua** també apareix atenuat aquí fins que totes les files tenen un docent triat o **Nou** marcat.

   ![Pantalla de Resoldre docents amb la casella Nou abans del desplegable Docent](../../assets/admin/working-schedules-import-02-resolve-teachers-new.png)
6. Una pantalla de **Docents pendents** llista cada docent que aquesta importació està a punt de crear com a empleat nou, pendent d'identificació — els codis de lloc encara no cobert i qualsevol correu amb **Nou** deixat marcat a la pantalla anterior. El destí de cada docent ja queda decidit en aquest punt, així que aquesta pantalla no canvia independentment de com es resolguin els conflictes de més avall. No hi ha res a resoldre aquí, és només una previsualització del que està a punt de passar — fes clic a **Continua**. Si el lot no crearà cap docent nou, veuràs un missatge de confirmació en lloc d'una llista.
7. Si dos docents diferents del mateix lot acaben programats a la mateixa aula i la mateixa hora, una pantalla de **Conflictes del fitxer** en llista cada parella (les dues columnes "Fitxer" són entrades del fitxer que estàs important) amb una manera de resoldre-ho: **"Confirmar"** si realment comparteixen aquella classe (l'opció per defecte quan és la mateixa assignatura i grup - només s'ofereix per a aquest cas exacte, no apareix en absolut als altres); **"Reassignar aules"** per a un xoc real d'aula - una classe desdoblada ("Sessió desdoblada") que necessita dues aules diferents (mateixa assignatura, grups diferents) o dues classes completament no relacionades que coincideixen per casualitat (assignatures diferents, "Conflicte d'aula") - tria l'aula real per a cada costat, ja que totes dues comencen amb la mateixa aula que provoca el conflicte (l'aula ja assignada a cada grup); o **"Preval l'esquerra"/"Preval la dreta"** per mantenir simplement un costat i descartar l'altre. Cada fila es tenyeix segons la urgència: groc per a una fila de co-docència/classe desdoblada que només demana una confirmació ràpida, vermell per a un xoc d'aula real que exigeix una decisió de veritat. Si no hi ha res a resoldre, veuràs un missatge de confirmació. El botó **Continua** apareix atenuat fins que totes les files tenen una resolució real (per a "Reassignar aules", això vol dir que les dues aules han de ser realment diferents).

   ![Pantalla de Conflictes del fitxer amb una fila de classe desdoblada tenyida en groc](../../assets/admin/working-schedules-import-03-internal-conflicts-colors.png)
8. Si alguna entrada del fitxer coincideix amb una aula+hora ja utilitzada activament per l'horari existent d'algú altre, una pantalla de **Conflictes amb horaris existents** en llista cadascuna (amb el mateix tenyit groc/vermell) - aquí les columnes es diuen **"Fitxer"** (la nova entrada) i **"Base de dades"** (la sessió ja existent), en lloc dels dos "Fitxer" de la pantalla anterior - amb les mateixes opcions de resolució que "Conflictes del fitxer" més amunt. Triar **"Preval l'esquerra"** arxiva la sessió existent (alliberant l'espai per a la nova); triar **"Preval la dreta"** descarta la nova entrada en lloc d'això, deixant la sessió existent intacta. Un cas té un valor per defecte diferent: un docent ja amb doble reserva en el *temps* contra el seu propi horari existent, en una aula genuïnament diferent (no és un xoc d'aula en absolut - reassignar aules no soluciona que un docent hagi d'estar en dos llocs alhora), per defecte usa **"Preval l'esquerra"** sense selectors d'aula, en lloc de "Reassignar aules". Si no hi ha res a resoldre, veuràs un missatge de confirmació.
9. Una pantalla de **Resum general** recapitula tota l'operació abans de confirmar-la: un recompte de cada nom de grup, correu de docent, docent pendent i conflicte resolts durant el procés, més una llista de cada docent que aquesta importació ja ha aparellat amb un empleat real i existent (reconegut automàticament pel correu, o corregit a la pantalla "Resoldre docents") — un avís de que aquesta importació està a punt d'actualitzar (sobreescriure) el seu horari/assignacions d'assignatures. Si cap dels docents del fitxer existeix ja, veuràs un missatge de confirmació en lloc d'aquesta llista. Com que cap dels passos anteriors permet tornar enrere, aquesta és la darrera oportunitat de comprovar que tot és correcte abans de fer clic a Importa.
