...
The reasons teachers pick from when issuing a strike are configurable under **Convivencia → Configuration → Strike reasons**.
...
- Untick **Active** to retire a reason without deleting it (existing strikes keep referencing it).
- The seeded "Other / General" reason (`ems.strike_reason_other`) is the system default — keep it active, since it's what the roll-call dialog preselects.
