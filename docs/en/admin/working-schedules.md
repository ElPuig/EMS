...
6. A **Pending teachers** screen lists every teacher this import is about to create as a new, pending-identification employee — the not-yet-hired post codes and any e-mail with **New** left ticked on the previous screen. Every teacher's fate is already fully decided by this point, so this screen never changes regardless of how the conflicts further below get resolved. Nothing to resolve here, just a preview of what's about to happen — click **Continue**. If nothing in the batch will create a new teacher, you'll see a confirmation message instead of a list.
7. If two different teachers in the same batch end up scheduled in the same classroom at the same time, a **File conflicts** screen lists each colliding pair (both the "File" columns are entries from the file you're importing) with a way to resolve it: **"Confirm"** if they're genuinely sharing that class (the default when it's the same subject and group — only offered for this exact case, not shown at all for the others); **"Reassign rooms"** for a real room clash — a split ("Split session") needing two different rooms (same subject, different groups) or two entirely different classes that happen to collide (different subjects, "Room conflict") — pick the actual room for each side, since both start pre-filled with the same colliding one (their group's own currently-assigned classroom); or **"Left prevails"/"Right prevails"** to simply keep one side and drop the other instead. Each row is tinted by how urgent it is: yellow for a co-teaching/split-session row that's only asking for a quick confirmation, red for a genuine room clash that needs an actual decision. If there's nothing to resolve, you'll see a confirmation message instead. **Continue** shows grayed out until every row has an actual resolution (for "Reassign rooms", that means the two rooms must actually differ).
...
9. An **Overall summary** screen recaps the whole run before you commit to it: a count of every unresolved group name, teacher e-mail, pending teacher, and conflict resolved along the way, plus a list of every teacher this import already matched to a real, existing employee (whether recognized automatically by e-mail or corrected on the "Resolve teachers" screen) — a heads-up that this import is about to update (override) their schedule/subject assignments. If none of the file's teachers already exist, you'll see a confirmation message instead of that list. Since none of the earlier steps let you go back, this is your last chance to check everything looks right before clicking Import.
