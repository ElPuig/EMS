...
5. If the files mention a teacher e-mail EMS couldn't find, a **Resolve teachers** screen lists each one: either pick the real teacher from the dropdown (a typo/mismatch of an already-existing teacher), or, if the e-mail genuinely belongs to a teacher who has never been hired before, tick **Create new teacher** instead — this locks the dropdown (you can't do both) and creates a brand-new pending-identification teacher for that e-mail at the final Import step, the same way a not-yet-hired post code does (see "Teachers Not Yet Hired" below), except the file's own e-mail is kept and pre-filled as their **Work email**, editable by hand (**Assign corporate email manually** ticked) rather than auto-generated, since it hasn't been confirmed yet. If every e-mail was recognized, you'll see a confirmation message instead. **Continue** shows grayed out here too until every row has either a teacher picked or **Create new teacher** ticked.
6. If two different teachers in the same batch end up scheduled in the same classroom at the same time, an **Internal conflicts** screen lists each colliding pair with a way to resolve it: **"It's co-teaching"** if they're genuinely sharing that class (the default when it's the same subject and group); **"Reassign rooms"** if it's really a split ("desdoble") class needing two different rooms (the default when it's the same subject but different groups) — pick the actual room for each side, since both start pre-filled with the same colliding one; or **"Left prevails"/"Right prevails"** to simply keep one side and drop the other (the only options for two genuinely unrelated classes that happen to collide). If there's nothing to resolve, you'll see a confirmation message instead. **Continue** shows grayed out until every row has an actual resolution (for "Reassign rooms", that means the two rooms must actually differ).
7. If any entry from the file collides with a classroom+time already actively used by someone else's existing schedule, an **Existing schedule conflicts** screen lists each one, with the exact same 4 resolution options as "Internal conflicts" above ("It's co-teaching" / "Reassign rooms" / "Left prevails" / "Right prevails") - the only difference is "right" here means the already-existing session rather than another entry from the same file. Choosing **"Left prevails"** archives the existing session (freeing the slot for the new one); choosing **"Right prevails"** drops the new entry instead, keeping the existing session untouched. If there's nothing to resolve, you'll see a confirmation message instead.
