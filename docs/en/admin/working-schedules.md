...
5. If the files mention a teacher e-mail EMS couldn't find (a genuine typo/mismatch — not a not-yet-hired post code, see "Teachers Not Yet Hired" below), a **Resolve teachers** screen lists each one the same way: pick the real teacher from the dropdown (you can't create a new one here — a brand-new teacher is only ever created automatically for a not-yet-hired post code, at the final Import step). If every e-mail was recognized, you'll see a confirmation message instead. **Continue** shows grayed out here too until every row has a teacher picked.
