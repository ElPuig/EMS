...
7. Click **Continue** through the remaining steps (not yet built as their own screens — a future version will let you resolve conflicts against already-existing schedules here interactively instead of only at the final step).
8. On the final step, click **Import**. This is the point where everything is actually written, and where any remaining problem (a missing classroom, a schedule conflict against an existing schedule) is reported, naming exactly what needs fixing.
