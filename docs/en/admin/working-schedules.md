...
6. Click **Continue** through the remaining steps (not yet built as their own screens — a future version will let you resolve room conflicts here interactively instead of only at the final step).
7. On the final step, click **Import**. This is the point where everything is actually written, and where any remaining problem (a missing classroom, a schedule conflict) is reported, naming exactly what needs fixing.
