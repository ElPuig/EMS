...
**`_sql_constraints` uniqueness on `(student_id, group_id, subject_id)` — fixed 2026-07-30.** The production database had 21 pre-existing duplicate triples (42 rows), all field-identical within their pair (only `id`/timestamps differed — no data-merge needed). Fixed with `('unique_student_group_subject', 'UNIQUE(student_id, group_id, subject_id)', ...)`. Since duplicates already existed, `migrations/18.0.0.22.0/pre-migrate.py::_dedupe_ems_enrollment` deletes the higher-id row of each duplicate pair via raw SQL **before** Odoo's schema sync tries to create the constraint — deliberately not through ORM `unlink()`, since `unlink()`'s `_ems_sync_grade_session_remove` has no "is the student still enrolled via another row" guard (unlike its sibling `_ems_sync_attendance_template_remove`, which does — see `plans/grade_session_remove_missing_still_enrolled_guard.md` for that separate, still-open latent bug) and would risk wiping the surviving duplicate's grade lines for an open session. Tested in `tests/test_enrollment.py::test_duplicate_student_group_subject_raises`/`test_same_student_different_group_is_allowed`. Full history in `plans/enrollment_junction_duplicate_constraint.md` (delete once this note fully supersedes it).
...
- **Attendance template remove:** only drops the student if **no other** remaining enrollment still keeps them within that same template's scope (`search_count` over `group_id in template.group_ids`) — otherwise a co-teaching student would be wrongly dropped from a template still covering one of their other groups.
- **Grade session add/remove:** only touches sessions in `state = 'open'` — `board`/`final` sessions are frozen and must not gain or lose lines from a later enrollment change.
