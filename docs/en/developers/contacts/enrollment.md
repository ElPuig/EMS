...
**No `_sql_constraints` uniqueness on `(student_id, group_id, subject_id)`.** This is a known, *not yet fixed* gap: the production database already contains duplicate rows for the same triplet (confirmed via a direct query during this DTON pass — dozens of pre-existing duplicates). Adding a uniqueness constraint now would fail the `ALTER TABLE` step on upgrade against that existing data. Deduplicating the live data first is a prerequisite for adding the constraint, and is a data-quality decision for the maintainers (which duplicate to keep, whether any downstream state — attendance/grade lines — was created twice as a result) rather than something to silently script during a documentation/testing pass. Flagged here for whoever picks this up next.
