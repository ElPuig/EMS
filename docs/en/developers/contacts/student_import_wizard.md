...
- **Family contacts are recognised by document, then by mobile.** `_get_or_create_family` uses the shared `res.partner._ems_find_family()` (issue #507, see [contact data requests](contact_data_request.md#recognising-a-family-contact)): by `document_id`/`passport_id` first, otherwise by mobile/phone number, accepted only when a single family contact holds it and the first name (`Tutor N - nom`) is compatible. A number held by someone else under another name (parents sharing a phone) creates a new contact and adds a "possible duplicate" warning to the result summary. A tutor row with neither a document nor a phone still cannot be recognised and still warns. Locked in by `test_get_or_create_family_without_document_matches_by_mobile` and `test_get_or_create_family_shared_mobile_other_name_creates_new_with_warning`.
...
- **Tutor row with neither a document number nor a phone** (`_process_tutor`): nothing to recognise the person by, so a new family contact is created; the warning names the student and the tutor so it is reviewed by hand. A tutor sharing a phone with another family contact under a different name gets its own "possible duplicate" warning (see the lookup above).
