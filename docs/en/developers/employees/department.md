...
graph TD
    ...
    P["hr.department (top-level, e.g. VET, ESO/BTX, ASP)"] -->|manager_id, top_level_role| AM["Area Manager<br/>(Head of Studies/Deputy/Secretary)"]
    AM -->|"role_hos/role_dhos (group_head_of_studies) or role_secretary (group_secretary)"| AM
    AM -->|"parent_id (rule 4b: company.director_id)"| DIR
    ...
    S -->|parent_id| H
    H -->|"parent_id (rule 4a: parent's manager_id)"| AM
...
1. Every member of the department, **except its own Chief/Area Manager**, gets `parent_id` = the department's `seminar_head_id` (or `manager_id` directly if no `seminar_head_id` — see rule 5).
...
4. **Cross-department cascade**, two parts:
   - **4a.** A department's own Chief/Area Manager gets `parent_id` = the *parent* department's `manager_id`, if the parent department has one set.
...
flowchart LR
    ...
    B --> D["update_department_head_role() / update_area_manager_role() / update_seminar_head_role() on old+new heads"]
