...
graph LR
    ...
    TU --> HD["group_head_of_department"]
    HD --> HS["group_head_of_studies"]
    HS --> D["group_director"]
    D --> A["group_academic_admin"]
```

`group_head_of_department` was added identical to `group_tutor` (`implied_ids = [group_tutor]`, no extra rights of its own yet). `group_head_of_studies` implies `group_head_of_department` instead of `group_tutor` directly, so the chain is unbroken.
...
- **`data/main/ems.role_group_relationship.xml`** wires specific role catalog entries (from `data/cat/ems.role.csv`) to their security group, e.g. `role_tutor → group_tutor`, `role_dchieff → group_head_of_department`, `role_hos`/`role_dhos → group_head_of_studies`, `role_director → group_director`.
...
| `ems.group_head_of_department` | `ems.group_tutor` | Department head. Currently identical to Tutor — no additional rules or views reference it yet |
| `ems.group_head_of_studies` | `ems.group_head_of_department`, `hr_attendance.group_hr_attendance_manager` | Full read/write access to all employees' attendance records |
...
No new record rules or views were needed for `group_head_of_department`: because it implies `group_tutor`, every rule/view gated on `group_teacher` (with a `tutor_id` domain) or on `group_tutor` directly is automatically satisfied.

## Key Migrations

| Version | Change | File |
|---------|--------|------|
| 18.0.0.19.3 | Renamed role catalog id `role_dchieff_cs` → `role_dchieff` (was incorrectly scoped to a single department) and wired it to the new `group_head_of_department` | `migrations/18.0.0.19.3/pre-migrate.py` |
