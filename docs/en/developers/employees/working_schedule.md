...
- **Derived break** (view mode only): a break the teacher's own calendar has no real saved row for yet is filled in from `hr.employee._get_derived_break_entries()` — one per shift (morning/afternoon) they actually teach, taken from the *level* framework of the groups in their real teaching entries for that shift (first entry of the shift, day/hour order, decides the level on a genuine tie). Exposed to the widget as a **separate** field, `derived_break_attendance_ids` (own compute, own hidden `<field invisible="1">` in the view) — deliberately never merged into `schedule_attendance_ids` itself, because Edit mode's buffer-seeding (`_seedBufferFromEntries`) treats everything in that field as real, already-saved data that always wins over the framework baseline; mixing a derived row in there would let Edit silently "adopt" it as real on the next `Save`. `entriesForDay()` merges the two client-side, view-mode only, skipping any derived slot a real entry already occupies. Same dedup/merge happens server-side in `get_schedule_report_lines()` for the PDF. Renders identically to a real break (compact single-line block, see below) — there is no visual difference between an explicitly-saved break and a derived one.
