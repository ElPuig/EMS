...
graph TD
    ...
    A -->|non_teaching| NT["Selection: AC/BR/CM/CT/G/MM/MT/R/S/SC/TT/WIC"]
```
...
- `non_teaching` (Selection) — a non-teaching commitment instead of a subject: `AC` (Another Coordinations), `BR` (Break — patio), `CM` (Coordination Meeting), `CT`, `G` (Guard), `MM`, `MT`, `R`, `S`, `SC`, `TT`, `WIC`.
...
- **Hours summary** (below the grid, view mode only, hidden while editing): `resource.calendar.get_schedule_hours_summary()` returns two columns — mirrors the real external schedules this data is modelled on:
  - **"Weekly teaching hours"**: rows grouped by `ems.group.level_id` (teaching periods), plus any non-teaching activity that isn't a guard duty or a Wednesday coordination meeting.
  - **"Other fixed-schedule hours"**: guard duties (`non_teaching == 'G'`, any day) and coordination meetings (`non_teaching == 'CM'`) specifically on Wednesday (`dayofweek == '2'`) — the centre's fixed non-teaching commitments. The break (`'BR'`) is dropped from both columns entirely.
