...
**Tasks.** Requests are put in a to-do list through `mail.activity.type`'s Task Assignment configuration (`_ems_get_task_users`), the same mechanism student documents and enrollment comments use: `ems.mail_activity_convalidation_review` on creation (Head of Studies) and `ems.mail_activity_convalidation_registration` on validation (secretariat). Each step closes the previous task (`_ems_close_tasks`). Assignees are unsubscribed from the thread, so their only notice is the task itself. Nothing is scheduled when nobody is configured — the helper logs a warning and moves on.
