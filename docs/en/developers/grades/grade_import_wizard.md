...
## Result HTML escaping (DTON pass, 2026-07-28)

`_build_result_html()` used to interpolate `stats["warnings"]`/`stats["errors"]` into HTML with
no escaping — the same bug found and fixed in all the other `*_html`-building import wizards
...
established fix pattern). It was concretely exploitable here: an error message echoes raw
uploaded-file content — `_log_error`'s "Student not found (idAlumne %s)" embeds `idalu`, read
straight from the xlsx's own `idAlumne` column, into an admin-only readonly `Html` field.

Fixed with the established pattern: `markupsafe.Markup(...).format(...)` (auto-escapes plain
`str` arguments) plus `ems.base.build_html_list()` for the warnings/errors list. The gotcha
documented for the other three wizards applies verbatim: a plain `''.join(...)` over a
generator of `Markup` fragments silently downgrades the result back to `str`, losing the
"already safe" marker, so the outer `.format()` re-escapes it and shows literal `&lt;li&gt;`
tags instead of a list. No text content changed, so this needed no new `.po` work.

In the same pass the class was renamed `ems_grade_import_wizard` → `EmsGradeImportWizard` and
the whole file, which was tab-indented, normalized to spaces.
