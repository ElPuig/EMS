...
| [Department Chief / Seminar Chief / Head of Studies cascade](employees/department.md) | `hr.department.manager_id`/`seminar_head_id`/`is_top_level`/`top_level_role` driving `hr.employee.parent_id` (including between parent/child departments) and the `role_dchieff`/`role_seminar`/`role_hos`/`role_dhos` roles automatically |
