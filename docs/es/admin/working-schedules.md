...
5. Si los archivos mencionan un correo de docente que EMS no ha encontrado, una pantalla de **Resolver docentes** los lista: elige el docente real en el desplegable (un error/desajuste de un docente ya existente), o bien, si el correo pertenece realmente a un docente que nunca ha sido contratado, marca **Crear nuevo docente** en su lugar - esto bloquea el desplegable (no se pueden hacer ambas cosas) y crea un nuevo docente pendiente de identificar para ese correo en el paso final de Importar, igual que un código de puesto aún no cubierto (ver "Docentes aún no contratados" más abajo), con la diferencia de que se conserva el correo del archivo, precargado como **Correo de trabajo** editable a mano (**Asignar correo corporativo manualmente** marcado) en lugar de generarse automáticamente, ya que todavía no se ha confirmado. Si todos los correos se reconocieron, verás un mensaje de confirmación. El botón **Continuar** también aparece atenuado aquí hasta que todas las filas tengan un docente elegido o **Crear nuevo docente** marcado.
6. Si dos docentes distintos del mismo lote acaban programados en la misma aula a la misma hora, una pantalla de **Conflictos internos** lista cada pareja con una forma de resolverlo: **"Es co-docencia"** si realmente comparten esa clase (opción por defecto cuando es la misma asignatura y grupo); **"Reasignar aulas"** si en realidad es una clase desdoblada que necesita dos aulas distintas (opción por defecto cuando es la misma asignatura pero grupos distintos) - elige el aula real para cada lado, ya que ambos empiezan con la misma aula que provoca el conflicto; o **"Prevalece la izquierda"/"Prevalece la derecha"** para simplemente mantener un lado y descartar el otro (las únicas opciones para dos clases genuinamente no relacionadas que coinciden por casualidad). Si no hay nada que resolver, verás un mensaje de confirmación. El botón **Continuar** aparece atenuado hasta que todas las filas tengan una resolución real (para "Reasignar aulas", eso significa que las dos aulas deben ser realmente distintas).
7. Si alguna entrada del archivo coincide con un aula+hora ya usada activamente por el horario existente de otra persona, una pantalla de **Conflictos con horarios existentes** lista cada una, con exactamente las mismas 4 opciones de resolución que "Conflictos internos" más arriba ("Es co-docencia" / "Reasignar aulas" / "Prevalece la izquierda" / "Prevalece la derecha") - la única diferencia es que aquí "derecha" significa la sesión ya existente en lugar de otra entrada del mismo archivo. Elegir **"Prevalece la izquierda"** archiva la sesión existente (liberando el hueco para la nueva); elegir **"Prevalece la derecha"** descarta la nueva entrada en su lugar, dejando la sesión existente intacta. Si no hay nada que resolver, verás un mensaje de confirmación.
