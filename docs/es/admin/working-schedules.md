...
5. Si los archivos mencionan un correo de docente que EMS no ha encontrado (un error/desajuste real - no un código de puesto aún no cubierto, ver "Docentes aún no contratados" más abajo), una pantalla de **Resolver docentes** los lista igual: elige el docente real en el desplegable (aquí no se puede crear uno nuevo - un docente completamente nuevo solo se crea automáticamente para un código de puesto aún no cubierto, en el paso final de Importar). Si todos los correos se reconocieron, verás un mensaje de confirmación. El botón **Continuar** también aparece atenuado aquí hasta que todas las filas tengan un docente elegido.
...
7. Haz clic en **Continuar** en las pantallas restantes (todavía no construidas como pantallas propias — una versión futura permitirá resolver aquí mismo los conflictos contra horarios ya existentes, en lugar de solo en el paso final).
8. En el paso final, haz clic en **Importar**. Este es el momento en que todo se escribe de verdad, y donde cualquier problema pendiente (un aula que falta, un conflicto de horario contra un horario existente) se reporta indicando exactamente qué hay que corregir.
