[Català](../../ca/teachers/attendance-reports.md) | [Castellano](attendance-reports.md) | [English](../../en/teachers/attendance-reports.md)

---

# Informes de asistencia

**Asistencia → Informes** abre la pantalla de **Informes de asistencia**: una tabla dinámica que puedes explorar y exportar tú mismo, además de 3 informes PDF imprimibles (por grupo, por alumno, por asignatura) accesibles desde su cabecera.

**Rol necesario:** Profesor/a

---

## Explorar los informes de asistencia

1. Ve a **Asistencia → Informes**. Se abre directamente con una **tabla dinámica** — sin ningún clic adicional.
2. Por defecto las filas son alumnos y las columnas son estados de asistencia; usa la barra de búsqueda para filtrar por alumno, grupo, asignatura o estado, y **Agrupar por** para cambiar cómo se pliega la tabla.
3. Usa el icono de **hoja de cálculo/descarga** de la cabecera para exportar la tabla dinámica actual a Excel.
4. Cambia a la vista de **gráfico** (iconos arriba a la derecha) para ver un resumen visual — por defecto muestra el **% de absentismo por asignatura**, así puedes detectar de un vistazo qué asignaturas tienen más absentismo.

> A diferencia de los 3 informes PDF de abajo, esta pantalla no está limitada solo a los grupos/asignaturas que impartes — muestra todos los datos de asistencia a los que tienes acceso de lectura, igual que otras pantallas de asistencia para profesorado.

---

## Imprimir un informe PDF

1. En la pantalla de **Informes de asistencia**, haz clic en el icono **⚙ (engranaje)** de la cabecera y elige **Informe de asistencia (por grupo)**, **Informe de asistencia (por estudiante)** o **Informe de asistencia (por asignatura)**.
2. Selecciona un **Nivel**, luego un **Estudio** y luego un **Grupo** — cada selección filtra la siguiente. Los desplegables solo muestran los grupos y asignaturas que **realmente impartes**; si te falta algún grupo o asignatura, comprueba que estás asignado/a en tu horario docente.
3. En los informes por alumno y por asignatura, selecciona también el **Alumno**/la **Asignatura** — limitado a lo que realmente está matriculado o se imparte en el grupo seleccionado.
4. Las fechas **Desde**/**Hasta** se rellenan automáticamente con todo el rango de sesiones disponibles para tu selección; redúcelas si solo quieres un periodo concreto.
5. Haz clic en **Imprimir**. El PDF se abre con un resumen global de asistencia/absencia, un recuento por estado y las notas de sesión registradas durante el periodo.

---

[← Volver a los manuales de Profesores](index.md)
