...
def _fix_default_schedule_framework(cr):
    ...
    if not framework:
        _logger.warning("Migration 18.0.0.21.0: 'ems.schedule_framework_default' not found, nothing to fix.")
        return
    ...
    if companies:
        ...
        _logger.info(
            "Migration 18.0.0.21.0: corrected default_schedule_framework_id on %d company(ies).",
            len(companies),
        )
    ...
    for company in env['res.company'].search([]):
        ...
        if employees:
            _logger.info(
                "Migration 18.0.0.21.0: moved %d employee(s) off '%s' onto personal framework-seeded calendars.",
        ...
        if not still_used:
            try:
                with cr.savepoint():
                    old_calendar.unlink()
                _logger.info("Migration 18.0.0.21.0: deleted retired calendar '%s' (id=%d).", old_calendar.name, old_calendar.id)
            except Exception:
                _logger.warning("Migration 18.0.0.21.0: could not delete retired calendar id=%d, leaving it archived-in-place instead.", old_calendar.id)
...
def migrate(cr, _version):
    _fix_default_schedule_framework(cr)
