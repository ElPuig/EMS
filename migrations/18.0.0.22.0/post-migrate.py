...
def _drop_old_attendance_template_student_rel(cr):
    """ems.attendance_template.student_ids moved to ems.attendance_schedule.student_ids in this
    version (see plans/calendar_driven_attendance_templates.md, point 1). Odoo's own schema sync
    does NOT drop a Many2many's old relation table on its own just because the field was removed
    from the model (confirmed empirically 2026-08-11 - unlike a plain column, which IS dropped),
    so ems_attendance_template_res_partner_rel would otherwise sit around forever as orphaned,
    inaccessible garbage.

    No data is carried forward from it. An earlier version of this migration copied each
    (template, student) row onto the new per-line field first, via a pre-migrate backup + this
    function reading it back - abandoned (2026-08-12, confirmed by tracing the actual code, not
    assumed) once it became clear the copied data was always immediately discarded anyway:
    '_regenerate_attendance_templates_from_calendars' below (which this function's own migrate()
    call always precedes) archives every template the restored rows would have landed on and
    rebuilds a fresh set via 'fill_students()' (current ems.enrollment), which never reads
    anything this function could have restored - the whole backup/restore roundtrip did real work
    whose result nothing downstream ever consumed."""
    cr.execute("DROP TABLE IF EXISTS ems_attendance_template_res_partner_rel")
    _logger.info(
        "Migration 18.0.0.22.0: dropped the now-orphaned ems_attendance_template_res_partner_rel "
        "relation table - a schedule line's student roster is derived fresh from current "
        "enrollment (fill_students()), never carried forward from the old template-level field.")
...
def migrate(cr, _version):
    ...
    _drop_old_attendance_template_student_rel(cr)
