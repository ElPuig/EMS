...
class EmsPlanningOutcome(models.Model):
    ...
    def check_ponderation(self):
        for outcome_line in self:
            if outcome_line.ponderation < 0 or outcome_line.ponderation > 100:
                raise ValidationError(_("The ponderation value must be within the range [0,100]."))
