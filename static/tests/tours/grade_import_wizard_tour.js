...
registry.category("web_tour.tours").add("ems_grade_import_wizard_missing_sheet", {
    ...
    steps: () => [
        ...
        {
            // Matched by dialog class only, never by message text: this database runs in
            // Catalan/Spanish, so a ":contains('no gradeable rows')" trigger (the English
            // source string) would never match the rendered dialog.
