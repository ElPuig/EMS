...
#                                         (CI shards use this form - see compute_test_shards.py)
#   ./test.sh '-/ems:A,-/ems:B'        - same, but as an exclusion list (everything except A/B)
#
# The no-argument form parallelizes across the same shard split CI uses (compute_test_shards.py:
# one "fast" shard + N "tour" shards, self-maintaining - discovers tests/*_tour.py at run time,
# no script edit needed when a new tour file appears). See run_sharded_tests.py for how each
# shard gets its own throwaway database clone and HTTP port. A scoped run (single class or an
...
if [ -z "$1" ]; then
    python3 "$SCRIPT_DIR/run_sharded_tests.py"
    EXIT_CODE=$?
else
    TAG="/ems"
    case "$1" in
