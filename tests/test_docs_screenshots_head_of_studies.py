...
from .common import DocsScreenshotMixin, create_role_employee, create_role_user, next_student_id
...
class TestDocsScreenshotsHeadOfStudies(HttpCase, DocsScreenshotMixin):
    ...
    def setUpClass(cls):
        ...
        # --- Academic history ---
        cls.level, cls.study, cls.group = cls._level_study_group('DHOS', level_name='Formació professional')
        ...
        for name, result, rate in (('Laia Exemple', 'full', 96.5), ('Jordi Mostra', 'repeating', 78.2)):
            ...
            cls.env['ems.student.year_record'].create({
        ...
        cls.new_teacher = cls.env['hr.employee'].create({
            ...
            'work_email': 'teacher.exemple@example.com',
        })
    ...
    @classmethod
    def _level_study_group(cls, prefix, level_name):
        level = cls.env['ems.level'].create({'acronym': prefix, 'name': level_name})
        study = cls.env['ems.study'].create({
            'code': f'{prefix}-01', 'acronym': prefix, 'name': f'Estudi {prefix}',
    ...
    def test_capture_head_of_studies_screenshots(self):
        ...
        self._capture(
            ...
            '.o_form_statusbar', 'hos-staff-management-create-account.png',
            login='doc_shot_hos', wait_for=".o_form_statusbar button:contains('Create Google account')",
        )
        self._capture(
